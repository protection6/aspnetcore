...
export async function fetchAndInvokeInitializers(options: Partial<WebAssemblyStartOptions>): Promise<JSInitializer> {
  if (options.initializers) {
    ...
    return new JSInitializer(/* singleRuntime: */ true, undefined, [], WebRendererId.WebAssembly);
  }
}
